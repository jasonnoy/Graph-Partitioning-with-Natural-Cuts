...
//            ths.push_back(thread(&MultiLayerPartition::dealCell, this, l, cur_layer, cells[i], cellCount, edgeCount, void_nodes, process_count));
            ths.push_back(thread(dealCell, i, ref(process_count), l, cur_layer, ref(cells[i]), ref(cellCount), ref(edgeCount), ref(void_nodes), ref(graph_edges), outPath, nodeNum, U, C, FI, M, L));
        }
        for (int i = 0; i < cells.size(); i++){
            ths[i].join();
