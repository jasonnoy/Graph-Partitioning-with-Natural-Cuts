...
G_Edge sw_edge_adapter( link_info_t linkInfo, unsigned int nid, map<NodeID, NodeID> *id_map) {
    auto map = *id_map;
    NodeID oid = linkInfo.sw_link_id, source = map[linkInfo.start_node_id], target = map[linkInfo.end_node_id];
    if (nid == 100) {
        cout<<"oid: "<<oid<<"source: "<<linkInfo.start_node_id<<"target: "<<linkInfo.end_node_id<<endl;
        cout<<"source id: "<<map[linkInfo.start_node_id]<<" target id: "<<map[linkInfo.end_node_id]<<endl;
    }
